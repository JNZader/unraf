...
def guess_meta(md_path: Path) -> list[str]:
    ...
    if 'introduccion' in name or 'diplomatura' in name:
        tags.append('Intro')
    if 'que-es-la-ia' in name:
        tags.append('IA')
    if 'big-data' in name or '5v' in name:
        tags.append('Big Data')
    if 'machine-learning' in name:
        tags.append('ML')
    if 'prompt' in name:
        tags.append('Prompting')
    if 'generativa' in name or '4e' in name:
        tags.append('IAG')
    if 'estrategia' in name:
        tags.append('Estrategia')
    if 'procesos' in name:
        tags.append('Procesos')
    if 'arquitectura' in name:
        tags.append('Arquitectura')
    if 'automatizacion' in name:
        tags.append('Automatización')
    if 'gestion-estrategica-datos' in name:
        tags.append('Datos')
    if 'etica' in name or 'privacidad' in name or 'legal' in name:
        tags.append('Ética & Legal')
    if 'competitivo' in name or 'competencia' in name:
        tags.append('Competencia')
    if 'deloitte' in name or 'mckinsey' in name or 'tendencias' in name or 'organizaciones' in name:
        tags.append('Tendencias')
    if 'frameworks' in name or 'cheatsheet' in name:
...
<title>{html.escape(TITLE_MAP[page])} — Apuntes DIATO</title>
<style>{CSS}</style>
<script>
MathJax = {{
  tex: {{
